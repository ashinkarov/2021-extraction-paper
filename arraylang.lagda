...
supports efficient compilation to multicore and GPU
architectures~\cite{sac-nbody}.  We introduce the key aspects of
...
indices within the index-space generated by the shape \AD{s}.  A valid
...
reconstruct the parameters that are normally marked as \AC{unknown}.
...
vectors and head/tail to eliminate them\footnote{Here we show
  ...
  implement rank-polymorphic cons/hd/tail in a way
...
}                                         int hd (int[.] xs) { return xs[0]; }
\end{lstlisting}
Finally, note that if we can extract \AD{Vec}s, we can extract
\AD{List}s into exactly the same target language constructs.
The only difference lies in
the analysis of the nesting of the type.  \AD{Ar} of \AD{Vec}
